...
description: "Configurez les en-têtes de politique de sécurité du contenu pour autoriser les ressources Mintlify tout en maintenant la sécurité avec des proxys inverses et des politiques réseau strictes."
keywords: ["Content Security Policy", "CSP", "directives", "en-têtes de sécurité", "pare-feu", "prévention des attaques XSS"]
---

La politique de sécurité du contenu (CSP) est une norme de sécurité qui aide à prévenir les attaques de cross-site scripting (XSS) en contrôlant les ressources qu&#39;une page web peut charger. Mintlify applique une CSP par défaut qui protège la plupart des sites. Si vous hébergez votre documentation derrière un proxy inverse ou un pare-feu qui remplace la CSP par défaut, vous devrez peut-être configurer des en-têtes CSP pour que les fonctionnalités fonctionnent correctement.

<div id="csp-directives">
  ## Directives CSP
</div>

Les directives CSP suivantes déterminent quelles ressources une page peut charger :

* `script-src` : détermine quels scripts peuvent s’exécuter
...
* `connect-src` : détermine les URL auxquelles il est possible de se connecter pour les appels d’API et les connexions WebSocket
...
| Domaine                               | Utilité                                    | Directive CSP                                                                         | Obligatoire |
| :------------------------------------ | :----------------------------------------- | :------------------------------------------------------------------------------------ | :---------- |
...
| `*.mintcdn.com`                       | Images, favicons                           | `img-src`, `connect-src`                                                              | Obligatoire |
| `cdn.jsdelivr.net`                    | Ressources emoji pour les images OG        | `script-src`, `img-src`                                                               | Obligatoire |
...
| `cdn.segment.com`                     | Analyses Segment                           | `script-src`, `connect-src`                                                           | Facultatif  |
| `plausible.io`                        | Analyses Plausible                         | `script-src`, `connect-src`                                                           | Facultatif  |
| `us.posthog.com`                      | Analyses PostHog                           | `connect-src`                                                                         | Facultatif  |
| `tag.clearbitscripts.com`             | Suivi Clearbit                             | `script-src`                                                                          | Facultatif  |
| `cdn.heapanalytics.com`               | Analyses Heap                              | `script-src`                                                                          | Facultatif  |
...
<Note>
  Incluez uniquement les domaines des services que vous utilisez. Supprimez tous les domaines d’analyses que vous n’avez pas configurés pour votre documentation.
</Note>
